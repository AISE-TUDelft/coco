...
class Telemetry(BaseModel):
    ...
    time_since_last_completion: Union[
        int, None
    ]  # the time since the last completion in milliseconds
    typing_speed: Union[
        int, None
    ]  # the typing speed of the user in characters per minute
    document_length: Union[int, None]  # the length of the document in characters
    cursor_relative_position: Union[
        float, None
    ]  # the position of the cursor relative to the document
...
class GenerateRequest(BaseModel):
    ...
    timestamp: (
        datetime.datetime
    )  # the timestamp of the request (in the user's timezone)
...
class VerifyRequest(BaseModel):
    ...
    ]  # the ground truth of the completion
...
class SessionRequest(BaseModel):
    ...
    project_language: Union[
        LanguageType, None
    ]  # see LanguageType in Types.py -> this would be the most used language in the project
    project_ide: Union[
        IDEType, None
    ]  # see IDEType in Types.py -> this would be the most used IDE in the project
    user_settings: Union[
        Dict, None
    ]  # see UserSettings in Types.py -> this would be the user's settings
...
class SurveyRequest(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {
            "examples": [{"session_id": "123aaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"}]
        }
    }
