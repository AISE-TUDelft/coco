...
def get_context_from_request(
    request: ActiveRequest, coco_version: str, app: FastAPI, store_context: bool
) -> ContextCreate:
    """
...
def get_ground_truths_from_request(
    request: ActiveRequest, query: QueryCreate
) -> List[GroundTruthCreate]:
    """
...
def get_generation_from_request(
    request: ActiveRequest, query: QueryCreate, app: FastAPI
) -> List[HadGenerationCreate]:
    """
        ...
        for completion_model in request.completions.keys():
            generations.append(
                HadGenerationCreate(
                    ...
                    generation_time=0,
                    shown_at=[
                        str(x) for x in request.completions[completion_model].shown_at
                    ],
                    ...
                    logprobs=[
                        ...
                        0.4,
                        0.1,
                    ],  # TODO: Fix this -> this is simply a placeholder for the time being
                )
            )
...
def add_active_request_to_db(
    ...
    logger.log(
        logging.INFO,
        f"Adding active request with id {request.request.request_id} to DB for user {user_id}",
    )
    try:
        if store_completions:
            context = get_context_from_request(
                request, server_version_id, app, store_context
            )
            add_context(db_session, context)
            logger.log(
                logging.INFO,
                f"Added context with id {context.context_id} to DB for active request with id {request.request.request_id}",
            )
            ...
            logger.log(
                logging.INFO,
                f"Added telemetry with id {telemetry.telemetry_id} to DB for active request with id {request.request.request_id}",
            )
            ...
            logger.log(
                logging.INFO,
                f"Added query with id {query.query_id} to DB for active request with id {request.request.request_id}",
            )
            ...
            logger.log(
                logging.INFO,
                f"Added {len(generations)} total generations to DB for active request with id {request.request.request_id}",
            )
            ...
            logger.log(
                logging.INFO,
                f"Added {len(ground_truths)} total ground truths to DB for active request with id {request.request.request_id}",
            )
        else:
            logger.log(
                logging.INFO,
                f"Skipping storing completions for active request with id {request.request.request_id} as per user preferences.",
            )
    ...
    logger.log(
        logging.INFO,
        f"Successfully added active request {request.request.request_id} to DB.",
    )
